...
CF1_return50 <- allregressions %>% filter(CF == CFs[1]) %>% slice(which.min(abs(GEV - Hist_return50))) %>% dplyr::select(return)
CF2_return50 <- allregressions %>% filter(CF == CFs[2]) %>% slice(which.min(abs(GEV - Hist_return50))) %>% dplyr::select(return)
